...
function onCreatePost()
    ...
    if enable then
        local epath = "animicons/"..dadName
        local ppath = "animicons/"..boyfriendName
        --? check if anim file exists
        if checkFileExists("images/"..epath..".png") then
            enemyHasAnim = true
            loadGraphic("iconP2", epath, 150, 150)
            addAnimation('iconP2', 'idle', {1, 3, 2, 0, 5, 4}, false, 0) -- idle, lose, left, down, up, right
        end

        if checkFileExists("images/"..ppath..".png") then
            playerHasAnim = true
            loadGraphic("iconP1", ppath, 150, 150)
            addAnimation('iconP1', 'idle', {1, 3, 2, 0, 5, 4}, false, 0) -- idle, lose, left, down, up, right
