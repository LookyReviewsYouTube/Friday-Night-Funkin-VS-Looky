...
vocals = {{{{"Ee"}, "e", {''}}, {{"Oo"}, "o", {''}}, {{"Aa"}, "a", {""}}, {{"O"}, "o", {"h"}}}, -- dad, eee, ooo ,aaa ,e
...
local colorsGoogle = {"008744", "0057E7", "D62D20", "FFA700", "00cf68", "2b7bff", "ed4a3e", "ffc24f" -- flash
}
...
function onCreate()
    ...
    if shadersEnabled then
        ...
        makeLuaSprite('GlitchShader', '', 0.0001, -400);
        setSpriteShader("GlitchShader", "GlitchShader")
        makeLuaSprite("lensEffect", "", 0, -400)
        setSpriteShader("lensEffect", "lensDis")
        makeLuaSprite("radialBlurEffect", "", 0, -400)
        setSpriteShader("radialBlurEffect", "radialBlur")
        makeLuaSprite("chrEffect", "", 0, -400)
        setSpriteShader("chrEffect", "RGB_PIN_SPLIT")
    ...
    createVideos('lvideo3', 200, 0, 0.098, 0.098, 0.4)
    setProperty('lvideo3.colorTransform.alphaMultiplier', 0.15)
    setShaderFloat('lvideo3', "ySpeed", 0.15)
    createVideos('rvideo3', 920, 0, 0.098, 0.098, 0.4)
    setProperty('rvideo3.colorTransform.alphaMultiplier', 0.15)
    setShaderFloat('rvideo3', "ySpeed", 0.15)
    ...
    for i = 1, 9 do
        ...
        if i == 1 then
            TWSMinX = getProperty(tag .. '.x')
        end
        TWSMaxX = getProperty(tag .. '.x')
        setProperty(tag .. '.alpha', 0)
    end

    createVideos('lvideo2', 110, -20, 0.1562, 0.1562, 0.55)
    setProperty('lvideo2.colorTransform.alphaMultiplier', 0.3)
    setShaderFloat('lvideo2', "ySpeed", 0.3)
    createVideos('rvideo2', 940, -20, 0.1562, 0.1562, 0.55)
    setProperty('rvideo2.colorTransform.alphaMultiplier', 0.3)
    setShaderFloat('rvideo2', "ySpeed", 0.3)

    setProperty('lvideo2.x', getProperty('lvideo2.x') - 700)
    setProperty('rvideo2.x', getProperty('rvideo2.x') + 700)
    setProperty('lvideo3.x', getProperty('lvideo3.x') - 700)
    setProperty('rvideo3.x', getProperty('rvideo3.x') + 700)
    ...
    for i = 1, 7 do
        ...
        if i == 1 then
            TWMinX = getProperty(tag .. '.x')
        end
        TWMaxX = getProperty(tag .. '.x')
        setProperty(tag .. '.alpha', 0)
    end

    createVideos('lvideo1', -20, -10, 0.25, 0.25, 0.7)
    setProperty('lvideo1.colorTransform.alphaMultiplier', 0.6)
    setShaderFloat('lvideo1', "ySpeed", 0.4)
    createVideos('rvideo1', 970, -10, 0.25, 0.25, 0.7)
    setProperty('rvideo1.colorTransform.alphaMultiplier', 0.6)
    setShaderFloat('rvideo1', "ySpeed", 0.4)
    ...
    SpriteUtil.makeText({
        tag = "googleChat",
        text = "(listening...)",
        font = "Google/Product Sans Regular.ttf",
        align = "left",
        ...
        borderQuality = 0,
        color = "000000",
        x = getProperty("googleBox.x") + 30,
        y = getProperty("googleBox.y") + 46,
        width = 800
    })
    runHaxeCode([[
    ...
    SpriteUtil.makeSprite({
        tag = "deathPCBG",
        graphicColor = "FFFFFF",
        graphicWidth = 1280 * 0.9,
        graphicHeight = 720 * 0.85
    })
    ...
    local jsonPath = "data/big-problem/" .. (getProperty("curSong")):lower() .. ".json"
    songData = loadJsonFile(jsonPath)
end
...
function onCreatePost()
    ...
    -- HUD IN GAME NO WAY
    local hpStuff = {'healthBar', 'healthBarBG', 'iconP1', 'iconP2'}
    ...
    if not SpecialNotes then
        for i = getProperty('unspawnNotes.length') - 1, 0, -1 do
            if getPropertyFromGroup("unspawnNotes", i, "noteType") == "Hurt Note" and
                getPropertyFromGroup("unspawnNotes", i, "mustPress") then
...
function genParticles(amount, speed)
    if particleAmount > 200 or (lowQuality and getRandomBool(70)) then
        return
    end
    ...
    if not inGameOver then
        ...
        if videoShaderUpdate then
            for i, v in pairs(shaderUpdate) do
                if (not halfVideoDes and (string.find(v, 'video3') or string.find(v, 'video2'))) or
                    (string.find(v, 'video0') or string.find(v, 'video1')) then
        ...
        if screamMode then
            ...
            setProperty('camFollow.x', (not centerCamera) and (targetXCam[mustHitSection and 2 or 1]) or
                (math.abs(targetXCam[2] + targetXCam[1]) / 2 - 40))
        end
        ...
        if shadersEnabled then
            ...
            if getShaderFloat("lensEffect", "intensity") ~= 0 then
                setShaderFloat("lensEffect", "intensity",
                    lerp(getShaderFloat("lensEffect", "intensity"), 0, elapsed * 8))
            end
        end
        ...
        if not EasyMode and curStep >= 512 and curStep < 767 then
...
function onStepHit()
    ...
    if curStep == 640 then
        ...
        for i = 2, 3 do
            ...
            cancelTween(tagl)
            setProperty(tagl .. '.x', videoX['lvideo' .. i])
            cancelTween(tagr)
            setProperty(tagr .. '.x', videoX['rvideo' .. i])
        end
    end
    ...
    if curStep == 1280 then -- TWITTER ready
        ...
        setProperty('lvideo0.visible', false)
        setProperty('rvideo0.visible', false)
        setProperty('lvideo1.visible', false)
        setProperty('rvideo1.visible', false)
        setProperty('lvideo2.visible', false)
        setProperty('rvideo2.visible', false)
    ...
    if curStep == 1824 then -- phase 2 youtube
        ...
        setProperty('lvideo0.visible', true)
        setProperty('rvideo0.visible', true)
        setProperty('lvideo1.visible', true)
        setProperty('rvideo1.visible', true)
        setProperty('lvideo2.visible', true)
        setProperty('rvideo2.visible', true)
    ...
    if curStep == 3648 then -- VIDEO AND HEALTH BAR DESTROYED
        ...
        for i, v in pairs(hpStuff) do
            ...
            if tag and getProperty(tag .. '.angle') ~= nil then
                local randomAngle = math.random() * 40 - 20 -- random float idfk :3
                local newAngle = getProperty(tag .. '.angle') +randomAngle
                doTweenAngle(tag .. 'An', tag, newAngle, fur, 'quadIn')
            else
                debugPrint("invalid fucking tag you bitch")
            end
        end
...
function onBeatHit()
    if (curBeat % getProperty('TW.danceEveryNumBeats') == 0 and getProperty('TW.animation.curAnim') ~= nil and
        not getProperty('TW.animation.curAnim.name'):find('sing') and not getProperty('TW.stunned')) then
...
function opponentNoteHit(membersIndex, noteData, noteType, isSustainNote)
    ...
    if screamMode and noteData % 4 == 2 then
        ...
        if not EasyMode and Mechanic then
            ...
            doTweenAngle('camHUDangle', 'camHUD', 0, 0.3, 'quadOut')
            setProperty('camGame.angle', angle)
            doTweenAngle('camGameangle', 'camGame', 0, 0.3, 'quadOut')
        end
    ...
    if noteType == '_TwitterSing' or (curStep >= 2080 and curStep < 2336) or (curStep >= 1312 and curStep < 1824) or
        (curStep >= 3712 and curStep < 3776) then
...
function goodNoteHit(membersIndex, noteData, noteType, isSustainNote)
    if noteType == "_GoogleSing" or (curStep >= 3712 and curStep < 3776) then
        sing(true, membersIndex, noteData, noteType, isSustainNote)
    end
    if screamMode then
...
function loadJsonFile(path) -- Ammar : "Why"
    ...
    local file = getTextFromFile(path) -- FUN FACT: this will just output text and not array :3

    runHaxeCode("setVar('fileJson', null);")
    setProperty('fileJson', file)
    addHaxeLibrary('Json', 'haxe')
    runHaxeCode([[
         var fil = getVar('fileJson');
        //setVar("fileJson", Json.parse(fil));
    ]])
    local parsed = getProperty('fileJson')
    return parsed
end
...
function sing(isPlayer, id, dir, type, sus)
    ...
    if getTextString(texter) == "(listening...)" or getProperty(texter .. ".textField.numLines") > 2 then
        setTextString(texter, "")
    end
    ...
    if true then
        -- if type(numVocalName) ~= "string" then
        --     error("numVocalName must be valid string goober :3")
        -- end

        if id == nil then
            debugPrint("id must be a valid number goob 3:<")
            return;
        end

        local vocalsData = songData.vocals or {}

        local singData = vocalsData[id]
        if not singData then
            singData = 0 --default to 0 if no data
        end

        _G[numVocalName] = singData +1
    ...
    local notSusAtAll = (not getPropertyFromGroup("notes", id, "nextNote.isSustainNote")) and
                            (not getPropertyFromGroup("notes", id, "isSustainNote"))
