...
function varTween(_tag, _var, fromValue ,toValue, duration, _ease, _scriptName)
   ...
   runHaxeCode([[
      ...
      chosenEase = getEase(chosenEase);
            ...
            game.callOnLuas('tweenEnded', ["]]..tag..[[Tween"]);
         }
		});

   ]])
   table.insert(activeTweens, tag..'Tween')
end

function tweenEnded(tag)
   for i = 1, #activeTweens do 
      if activeTweens[i] == tag then 
