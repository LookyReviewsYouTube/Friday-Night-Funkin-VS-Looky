...
function onStepHit()
    if mechanic then
        if curStep == 528  then
            for i = 4,7 do
                noteTweenX("noteX"..i, i, centerPosition[i+1], crochet/1000*16, "quadInOut")
            end
        end
        if curStep == 640  then
            for i = 4,7 do
                noteTweenX("noteX"..i, i, centerPosition[i%4+1], crochet/1000*4, "quadIn")
            end
        end
        if curStep >= 784 and curStep < 1040 then
            if curStep % 4 == 0 then
                for i = 4,7 do
                    noteTweenY("noteY"..i, i, defaultPosition[i+1].y + (math.sin(getSongPosition()/200 + i*1.25) * (downscroll and 70 or -70)), crochet/1000/2, "quadOut")
                end
            end
            if curStep % 4 == 2 then
                for i = 4,7 do
...
function onUpdate(elapsed)
    if not inGameOver and mechanic then
        ...
        for i = 0, getProperty("notes.length")-1 do
            ...
            if strumTime >= 50400 and strumTime < 58800 then
            ...
            if (strumTime >= 58800 and strumTime < 78000) or (strumTime >= 127200 and strumTime < 146400) then
                local offX = getProperty('noteOffset.x') + (math.sin(distance*0.5) * -getProperty('noteOffset.x') * 0.25)
