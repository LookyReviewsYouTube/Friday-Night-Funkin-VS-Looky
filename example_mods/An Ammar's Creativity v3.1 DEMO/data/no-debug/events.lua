...
function onCreatePost()
    v1 = getDataFromSave("ammarc", "mechanic");
    v2 = getDataFromSave("ammarc", "shaders") and getPropertyFromClass("ClientPrefs", "shaders");
end
function onBeatHit()
    if (curBeat == 4) then
    ...
    if ((curBeat == 71) and v1) then
        debugPrint("healthBar.visible = false;");
        setProperty("healthBar.visible", false);
        setProperty("healthBarBG.visible", false);
    end
    if ((curBeat == 74) and v1) then
        debugPrint("icons.visible = false;");
        setProperty("nIconP1.visible", false);
        setProperty("nIconP2.visible", false);
    end
    if ((curBeat == 77) and v1) then
        debugPrint("LIVES = 3;");
        local v4 = 3;
        setGlobalFromScript("scripts/engine/engine", "LIVES", v4);
        setGlobalFromScript("scripts/engine/engine", "curLive", v4);
        setTextString("missText", "LIVES: " .. v4 .. "/" .. v4);
    ...
    if (curBeat == 86) then
        debugPrint(((getDataFromSave("ammarc", "highscore")["no-debug"] == nil) and
                       '\"I will let you do debugging stuff\"') or '\"I will let you do uhh. idk. You already beat me\"');
    end
    if ((curBeat == 90) and (buildTarget == "android") and v1) then
        debugPrint('\"Also, since you are on mobile, I\'ll give you 5 LIVES\"');
        local v5 = 5;
        setGlobalFromScript("scripts/engine/engine", "LIVES", v5);
        setGlobalFromScript("scripts/engine/engine", "curLive", v5);
        setTextString("missText", "LIVES: " .. v5 .. "/" .. v5);
    end
    if (curBeat == 266) then
